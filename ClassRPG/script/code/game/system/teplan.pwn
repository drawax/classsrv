...
CMD:tozsde(playerid, params[])
{
	...
	if(egyezik(type, "aktiválás"))
	{
		...
		LezerBekapcs();
		//SendRadioMessageFormat(FRAKCIO_KATONASAG, COLOR_DBLUE, "** Értesítés: %s  aktiválta Las Venturas-i Tõzsde védelmi rendszerét", ICPlayerName(playerid));
	}
		...
		elseif(egyezik(typeketto, "átír"))
		{
			if(IsACop(playerid) && Munkarang(playerid, 6))
			{
				if(sscanf(params, "{s[6]}{s[4]}c[32]", jelszoka))
					return Msg(playerid,"/jelszó Átír [jelszó]");
 	...
 	RandomTalalas = Rand(0,9);
  	switch(RandomTalalas)
  	{
   		case 0..4:
   		{
			...
			Cselekves(playerid,"feltörte a rendszert,de valamit elrontott", 0);
			foreach(Jatekosok, b)
			{
				if(IsACop(b) && OnDuty[b])
					//Még nincs koordi 0 0 0 helyére
					SetPlayerCheckpoint(b, 0, 0, 0, 1), SCM(b,COLOR_ALLDEPT,"** RIADÓ ** Betörtek a Las Venturasi Tõzsde Biztonsági rendszerébe!");
			}
