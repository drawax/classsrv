...
CMD:bonusz(playerid, params[])
{
	...
	if(PremiumInfo[playerid][pKamat] > 0 && PremiumInfo[playerid][pKamatIdo] > 0)
		SendFormatMessage(playerid, COLOR_WHITE, "Kamat: +%f százalék (még %d fizetésig)", PremiumInfo[playerid][pKamat] + FLOATFIX, PremiumInfo[playerid][pKamatIdo]);
	else
		Msg(playerid, "Kamat: nincs", false, COLOR_WHITE);
	
	if(PremiumInfo[playerid][pAdo] > 0 && PremiumInfo[playerid][pAdoIdo] > 0)
		SendFormatMessage(playerid, COLOR_WHITE, "Adócsökkentés: -%f százalék (még %d fizetésig)", PremiumInfo[playerid][pAdo] + FLOATFIX, PremiumInfo[playerid][pAdoIdo]);
	else
		Msg(playerid, "Adócsökkentés: nincs", false, COLOR_WHITE);
