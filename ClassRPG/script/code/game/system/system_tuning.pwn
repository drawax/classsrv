...
#define D_TUNING_EMP				206
#define D_TUNING_EMP_MEG			2061
#define D_TUNING_FELNI				207
#define D_TUNING_FELNI_MEG			2071
#define D_TUNING_MATRICA			208
#define D_TUNING_MATRICA_MEG		2081
#define D_TUNING_TUNINGPACK			209
#define D_TUNING_TUNINGPACK_MEG		2091
#define D_TUNING_EGYEB				210
#define D_TUNING_EGYEB_LE			2101
...
ShowVehicleEditor(playerid)
{
	...
	switch(D_Tuning[playerid])
	{
		...
		case D_TUNING_EMP:					CustomDialog(playerid, D:tuning, DIALOG_STYLE_LIST, "EMP kiválasztása", "0. szintû EMP (leszerelés)\n1. szintû EMP (20 százalék esély a blokkolásra - 5.000.000 Ft)\n2. szintû EMP (35 százalék esély a blokkolásra - 8.000.000 Ft)\n3. szintû EMP (50 százalék esély a blokkolásra - 12.000.000 Ft)", "Kiválaszt", "Mégse");
		case D_TUNING_EMP_MEG:			{	tformat(256,"Valóban fel szeretnéd szerelni a jármûre a következõt: EMP?\nA mûvelet %s forintba fog kerülni!", FormatInt(D_Tuning_ItemCost[playerid])); CustomDialog(playerid, D:tuning, DIALOG_STYLE_MSGBOX, "Kinézet", _tmpString, "Megerõsítés", "Mégse"); PlayerPlaySound(playerid,1133,0.0,0.0,0.0); }
...
Dialog:tuning(playerid, response, listitem, inputtext[])
{
	...
	if(!response)
	{
		switch(D_Tuning[playerid])
		{
			...
			case D_TUNING_FESTES, D_TUNING_RIASZTO, D_TUNING_NEON, D_TUNING_HIDRAULIKA, D_TUNING_DETEKTOR, D_TUNING_EMP, D_TUNING_FELNI, D_TUNING_MATRICA, D_TUNING_TUNINGPACK, D_TUNING_EGYEB, D_TUNING_EGYEB_LE:
			{
			...
			case D_TUNING_EMP_MEG: 			{ 	ResetTuningVariables(playerid); D_Tuning[playerid] = D_TUNING_EMP; ShowVehicleEditor(playerid); return 1; 		}
	...
	if(D_Tuning[playerid] == D_TUNING_ALAP)
	{
		switch(listitem)
		{
			...
			case 5: //EMP
			{
				...
				D_Tuning[playerid] = D_TUNING_EMP;
				ShowVehicleEditor(playerid);
			}
			case 6: //Felni
			{
	...
	elseif(D_Tuning[playerid] == D_TUNING_EMP)
	{
		switch(listitem)
		{
			case 0:
			{
				if(CarInfo[vs][cEMP] == 0) return ShowVehicleEditor(playerid);
				D_Tuning[playerid] = D_TUNING_EMP_MEG;
				D_Tuning_SelectedItem[playerid] = 0;
				ShowVehicleEditor(playerid);
			}
			case 1:
	...
	elseif(D_Tuning[playerid] == D_TUNING_EMP_MEG)
	{
		if(!BankkartyaFizet(playerid, D_Tuning_ItemCost[playerid], false)) { SendFormatMessage(playerid, COLOR_LIGHTRED, "ClassRPG: Nincs elég pénzed! A tuning ára %s Ft!", FormatInt(D_Tuning_ItemCost[playerid])); D_Tuning[playerid] = D_TUNING_EMP; D_Tuning_ItemCost[playerid] = 0; ShowVehicleEditor(playerid); JarmuTuning(vehicle); return 1; }
		CarInfo[vs][cEMP] = D_Tuning_SelectedItem[playerid];
