...
new const CustomDamage[WEAPON_DAMAGE_TYPES][customDamage] =
{
	...
	{     DAMAGE_DEFAULT,	0.0}, //  0  ököl
	{     DAMAGE_DEFAULT,	0.0}, //  1  bokszer
	{     DAMAGE_DEFAULT,	0.0}, //  2  golf club
	{     DAMAGE_DEFAULT,	0.0}, //  3  nightstick
	{     DAMAGE_DEFAULT,	0.0}, //  4  kés
	{     DAMAGE_DEFAULT,	0.0}, //  5  bézbózütõ
	{     DAMAGE_DEFAULT,	0.0}, //  6  ásó
	{     DAMAGE_DEFAULT,	0.0}, //  7  dákó
	{     DAMAGE_DEFAULT,	0.0}, //  8  katana
	{        DAMAGE_NULL,	0.0}, //  9  láncfûrész
	{     DAMAGE_DEFAULT,	0.0}, // 10  kétvégû dildo
	{     DAMAGE_DEFAULT,	0.0}, // 11  dildo
	{     DAMAGE_DEFAULT,	0.0}, // 12  vibrátor
	{     DAMAGE_DEFAULT,	0.0}, // 13  ezüst vibrátor
	{     DAMAGE_DEFAULT,	0.0}, // 14  virág
...
// akkor biztosított a láb, fejlövés, bizonyos mennyiség után
...
/*	///////////////////////
	...
	if(IsACop(attacker) && Tazer[attacker] && Sokkolt[attacker] == 0 && (WeaponArmed(attacker) == WEAPON_DEAGLE || WeaponArmed(attacker) == WEAPON_SILENCED) && OnDuty[attacker] && GetPlayerState(attacker) == PLAYER_STATE_ONFOOT)
	{
		...
		if(IsPlayerInWater(attacker) || IsPlayerInWater(playerid)) { Msg(attacker, "Vízben lévõt akarsz lesokkolni? Az áram agyonüt titeket!"); fail = 1; }
	...
	// RIFLE //
	///////////
	else if(damageType == 33 && Armour[playerid] <= 30.0 && !Paintballozik[playerid]) // rifle = 2 lövés 10 másodpercen belül = anim
	{			
	...
	if(Loterben[playerid] != NINCS && LoterInfo[Loterben[playerid]][lTalalt] == false)
	{
		...
		if(LoterInfo[l][lObject] == objectid)
		{
			...
			else
			{
				KillTimer(LoterInfo[l][lObjectTimer]);
				KillTimer(LoterInfo[l][lTimer]);
				SendClientMessage(playerid,COLOR_LIGHTGREEN,"[Lõtér] Lelõttél egy ellenséget! [1 PONT]");
