#if defined __game_system_amos
	#endinput
#endif
#define __game_system_amos

/****************************
 *           cmdk           *
 *    á: 1   ó: 4   ú: 7    *
 *    é: 2   ö: 5   ü: 8    *
 *    í: 3   õ: 6   û: 9    *  
 ****************************/

//*******************************************************************************************************
//Verzió V3.6
// /fme - cselekvés feliratot helyez a feje fölé, az összetettebb RP-k érdekében.
// /fdo - történés feliratot helyez a feje fölé, szintén az összetettebb RP-k érdekében.
// /fnv - névtelen cselekvést helyez a feje fölé, de az IDK ott lesz, így az adminok tudják ki az.
//(PL: ha szeretnéd, hogy észre vegyék, ha megsérültél, de nem ez az elsõdleges cél)
// /felszerelések pozíciója pontosan ugyan ott lesz ahol a duty hely, így IG átrakáskor ez is változik.
// Munkaügyi hivatal beiktatása (Tehát már nem a városházán van!)
// /sex paramcs ágyban is használható ténylegesen.
//*******************************************************************************************************