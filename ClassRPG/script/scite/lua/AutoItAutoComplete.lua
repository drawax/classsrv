...
SCLEX_PAWN				= 3
SCE_PAWN_COMMENT		= 1
SCE_PAWN_COMMENT_SINGLE	= 2	
SCE_PAWN_STRING			= 6
SCE_PAWN_CHARACTER		= 7
...
function AutoItAutoComplete:OnChar(c)
	...
	if self:IsLexer(SCLEX_AU3) or self:IsLexer(SCLEX_PAWN) then
	...
	--	end
		...
		-- Exit if style isnt correct
		if not self:IsValidStyle(style) then
			return
		end
