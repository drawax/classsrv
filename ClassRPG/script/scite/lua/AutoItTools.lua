...
function AutoItTools:OnStartup()
	...
	if self:FileExists(props['SciteDefaultHome'].."\\au3.keywords.properties") then
		os.remove (props['SciteDefaultHome'].."\\properties\\au3.keywords.properties")
		os.rename (props['SciteDefaultHome'].."\\au3.keywords.properties", props['SciteDefaultHome'].."\\properties\\au3.keywords.properties")
	end
end	-- OnStartup()
...
function AutoItTools:OpenInclude(version)
	...
	if place then
		IncFile = string.gsub(IncFile,"\<","")		
		IncFile = string.gsub(IncFile,"\>","")		
		IncFile1 = props['autoit3dir'] .. "\\" .. version .. "\\include\\" .. IncFile
		IncFile2 = IncFile
	else  -- Else it is a include file in the script dir 
		IncFile = string.gsub(IncFile,"\"","")		
		IncFile1 = string.gsub(IncFile,"\'","")		
		IncFile2 = props['autoit3dir'] .. "\\" .. version .. "\\include\\" .. IncFile1
	end
