...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	// Find the recording_name[] file in /scriptfiles/
 	if(strcmp(cmd, "/felvétel", true) == 0) {
	    new tmp[512];
      	tmp = strtok(cmdtext, idx);
		if(!strlen(tmp)) {
			SendClientMessage(playerid,0xFF0000FF,"Usage: /felvétel {name}");
			return 1;
		}
		if(!IsPlayerInAnyVehicle(playerid)) {
            SendClientMessage(playerid,0xFF0000FF,"Felvétel: Ülj be egy kocsiba.");
			return 1;
		}
		StartRecordingPlayerData(playerid,PLAYER_RECORDING_TYPE_DRIVER,tmp);
		SendClientMessage(playerid,0xFF0000FF,"Felvétel: indul.");
		return 1;
	}
	...
	// Find the recording_name[] file in /scriptfiles/
 	if(strcmp(cmd, "/ofrecord", true) == 0) {
		...
		if(!strlen(tmp)) {
 		...
 		if(IsPlayerInAnyVehicle(playerid)) {
            SendClientMessage(playerid,0xFF0000FF,"Felvétel: Leave the vehicle and reuse the command.");
			return 1;
		}
		StartRecordingPlayerData(playerid,PLAYER_RECORDING_TYPE_ONFOOT,tmp);
		SendClientMessage(playerid,0xFF0000FF,"Felvétel: kezdés.");
		return 1;
	}
	
	// Stop recording any data
	if(strcmp(cmd, "/felvételstop", true) == 0) {
		StopRecordingPlayerData(playerid);
		SendClientMessage(playerid,0xFF0000FF,"Felvétel: megállítva.");
		return 1;
	}
